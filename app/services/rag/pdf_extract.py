...
def _ocr_scanned_pages(file_bytes: bytes, page_indices: List[int]) -> Tuple[Dict[int, str], dict, bool]:
    ...
    for batch in _group_contiguous(page_indices, _OCR_BATCH_PAGES):
        ...
        # (see course_material.py) so exact per-page placement doesn't need
        # to survive past this function — landing a batch's combined text at
        # its first page's slot and leaving the rest blank keeps document
        # order correct without needing to split Gemini's response back
        # apart per page.
