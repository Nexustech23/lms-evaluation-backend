...
def _call_generate_content_with_retry(client: genai.Client, model: str, contents: list, config=None):
    """
    Shared retry/backoff for client.models.generate_content calls: up to 5 attempts on
    transient errors (503/UNAVAILABLE/429/RESOURCE_EXHAUSTED/quota), honoring a
    "Please retry in Ns" hint from the API when present. Returns the raw response object
    (callers that need more than just .text — e.g. finish_reason for truncation
    detection — can inspect it directly; generate_content_from_file below just extracts
    .text and usage_metadata from it).
...
def generate_html_from_prompt(
    ...
    response = _call_generate_content_with_retry(
        ...
        config=genai_types.GenerateContentConfig(
            ...
            # answer. Callers size that budget for the document they want
            # (_NOTES_LENGTH_TOKENS: 3000 for "5 Pages"), which is how
            # Claude's max_tokens behaves — measured, thinking ate 1352 of
            # 3000 and the HTML came back cut off mid-<style>. Truncated
            # HTML is worse than short HTML: an unterminated style block
            # swallows the rest of the document, so the iframe renders
            # blank rather than partial. Producing a formatted document
            # from an explicit prompt needs no chain-of-thought, so
            # thinking is disabled to give the whole budget to output and
            # keep this budget parameter meaning the same thing for both
            # providers.
...
def generate_content_from_file(
    ...
    """
    Send file bytes (PDF/image) directly to Gemini with a prompt.
