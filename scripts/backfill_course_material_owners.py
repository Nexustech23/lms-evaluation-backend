...
listing endpoint (course_material.py) and the by-id lookup that gates
roadmap grounding (rag/mongo_store.py's find_document_by_id). A document
carrying no owner_user_ids matches nobody, so without this backfill every
roadmap whose grounded_doc_id points at a pre-existing document would
silently lose its grounding and start generating ungrounded notes.
...
/api/course-material/upload that no roadmap ever used. The uploader's
identity was only ever held in a Redis job record with a one-hour TTL, so it
is genuinely unrecoverable. Those are left ownerless, which means they stop
appearing in anyone's listing — the intended outcome, since that listing
previously showed every institute's uploads to every authenticated user.
Re-uploading the same file re-establishes ownership for free: dedup is by
content hash, so it costs no reprocessing and simply adds the uploader to
owner_user_ids.
