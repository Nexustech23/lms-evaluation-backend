# ============================================================
# Coverage for the public MyCareerGuru registration endpoint
# (app/api/routers/mycareerguru_auth.py) — the unauthenticated counterpart
...
async def test_mycareerguru_register_requires_no_auth(client):
    # The whole point: unlike /register, this must work with zero session.
    resp = await client.post("/mycareerguru/register", json={
...
async def test_mycareerguru_register_creates_inactive_self_learner(client, test_db):
    email = _email("mcg")
    resp = await client.post("/mycareerguru/register", json={
...
async def test_mycareerguru_register_rejects_duplicate_email(client, test_db):
    email = _email("mcg")
    first = await client.post("/mycareerguru/register", json={
        "role": "self_learner", "fullName": "First", "email": email, "password": PASSWORD,
    })
    assert first.status_code == 200

    second = await client.post("/mycareerguru/register", json={
        "role": "self_learner", "fullName": "Second", "email": email, "password": PASSWORD,
    })
    assert second.status_code == 400


async def test_mycareerguru_register_rejects_short_password(client):
    resp = await client.post("/mycareerguru/register", json={
...
async def test_mycareerguru_register_rejects_blank_fullname(client):
    resp = await client.post("/mycareerguru/register", json={
...
async def test_mycareerguru_register_rejects_invalid_email(client):
    resp = await client.post("/mycareerguru/register", json={
...
async def test_mycareerguru_register_rejects_wrong_role(client):
...
async def test_mycareerguru_registered_account_cannot_login_until_approved(client, test_db, superadmin_client):
    email = _email("mcg")
    await client.post("/mycareerguru/register", json={
    ...
    # endpoint the Super Admin "MyCareerGuru Accounts" page calls), login
    # succeeds.
    user = await test_db["users"].find_one({"email": email})
